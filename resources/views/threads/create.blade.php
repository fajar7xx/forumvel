...
@section('content')
    <div class="row">
        <div class="well card shadow-sm p-2 bg-light">
            <form action="{{route('thread.store')}}" class="form-vertical needs-validation" method="POST" role="form" id="create-thread-form" novalidate>
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="thread" id="thread" cols="30" rows="10" class="form-control @error('thread') is-invalid @enderror">{{old('thread')}}</textarea>
                    @error('thread')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
